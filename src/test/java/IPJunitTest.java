...
public class IPJunitTest {
    ...
    void test() {
        ...
        Result<Input<Token>> rTokenReader = TokenReader.lexFile("./src/main/java/h1_ip/test.txt", IPLexer::new);

        Result<ST> rst = rTokenReader.map(tr->tr.stream()
                .foldRight(()->templates.getInstanceOf("lexFilter"), f));
